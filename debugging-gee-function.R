...
gnome_gee_simulation <- function(
  ...
  for (ngp_i in seq_along(npgsloci)) {
    ...
    for (i in 1:n_set) {
      ...
      #
      print(head(phdatmz_e))
      print(head(phdatdz_e))
      #
      ...
      #
      print(head(phdatmzL_e))
      print(head(phdatdzL_e))
      #
  ...
  return(list(power = final_gee_power, params = final_gee_estimates, mod1 = summary(egeeM1mzdzL),
              mod2 = summary(egeeM2mzdzL)))
}

test <- gnome_gee_simulation(ct = .01, si = .025, npgsloci = 10)

testmodel1 <- test$mod1
testmodel2 <- test$mod2

testmodel1$coefficients
testmodel2$coefficients

testmodel1$coefficients[2,3]
testmodel2$coefficients[2,3]
